...
const regex = /<\/button>\s*<Header coverPhotoUrl=\{coverPhotoUrl\} \/>\s*<SalesLinksSection \/>\s*<JobSection jobs=\{parsedJobs\} onRefresh=\{handleRefreshJobs\} isRefreshing=\{isRefreshingJobs\} \/>\s*<LogoSection[\s\S]*?<PipelineCalendar data=\{pipelineData\} \/>\s*<CertidoesSection \/>/;
...
if (match) {
    ...
    content = content.replace(match[0], newStructure);
