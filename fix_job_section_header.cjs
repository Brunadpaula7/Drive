...
const replacement = `
                <div className="flex items-center gap-4 mr-3">
                    ...
                    {lastSyncTime && (
                        <span className="text-xs font-medium text-stone bg-sand/50 px-2.5 py-1 rounded-md border border-sand/50 flex items-center gap-1.5" title="Última sincronização dos dados">
...
content = content.replace(/<h2[\s\S]*?<\/span>\s*\)\}\s*<\/h2>/, replacement.trim());
fs.writeFileSync('components/JobSection.tsx', content);
console.log("Fixed JobSection header layout");
